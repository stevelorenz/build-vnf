...
function master(args)
    ...
    --start pictureSend
    mg.startTask("pictureSend", txQueue, DST_MAC, PAYLOAD_LEN)
...
function pictureSend(txQueue, dstMac, payloadLen)

    local pic = io.open('/home/pktgen/mg_test/01-20170320211730-24.jpg', "rb")
    local picData = pic:read('*a')
    local numPkts = math.ceil(string.len(picData) / payloadLen)
    local dataSeg = {}
    for i = 1, numPkts do
        table.insert(dataSeg, string.sub(picData, (i-1)*payloadLen+1, i*payloadLen))
    end
    ...
    local mempool = memory.createMemPool(function(buf)
        buf:getUdpPtpPacket():fill{
            ...
            udpSrc = SRC_PORT,
            udpDst = DST_PORT,
            pktLength = payloadLen + 76
        }
    end)
    -- a bufArray is just a list of buffers from a mempool that is processed as a single batch
    local bufs = mempool:bufArray(numPkts)

    local segID = 1

    while mg.running() do -- check if Ctrl+c was pressed
        -- this actually allocates some buffers from the mempool the array is associated with
        bufs:alloc(payloadLen + 76)
