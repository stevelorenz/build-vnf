...
def init_xdp_progs(bpf, ifce_names, func_names):
    ...
    for ifce, func in zip(ifce_names, func_names):
        print("[INFO] Load and attach XDP func %s to interface %s" % (func, ifce))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="UDP segments processing with XDP")
    ...
    parser.add_argument("-i", type=str, help="Interface(s) name list. Delimiter=,")
    ...
    parser.add_argument(
        "--dst_mac", type=str, default="", help="Destination MAC address"
    )
    parser.add_argument(
        "--payload_size", type=int, default=128, help="UDP payload size in bytes."
    )
    parser.add_argument("--debug", action="store_true", help="Enable debugging mode.")
    parser.add_argument(
        "--cksum", action="store_true", help="Recalculate checksum by VNF self."
    )
    ...
    if len(ifce_lst) == 1:
        ...
        print(
            "[INFO] Ingress interface: %s, egress interface: %s"
    ...
    if opt == 0:
        ...
        *pt_pload_64 = (*pt_pload_64 ^ 0x%s);
        ...
        print("[INFO] Align: %dB, Remainder: %dB, Num8B: %d" % (ALIGN_OFFSET, rmd, num))
