...
static void get_ip_str(uint32_t ip, char *ip_addr_str)
{
	...
	sprintf(ip_addr_str, "%d.%d.%d.%d", octet[3], octet[2], octet[1],
		octet[0]);
}
...
__attribute__((unused)) static int8_t filter_ether_frame(struct rte_mbuf *m)
{
	...
	for (i = 0; i < MAX_VNF_ETHPORTS; ++i) {
		if (is_same_ether_addr(&(ethh->s_addr),
				       &(l2fwd_ports_eth_addr[i])))
			return -3;
	}
...
int main(int argc, char **argv)
{
	...
	RTE_LOG(INFO, USER1,
		...
		max_poll_short_try, poll_short_interval_us,
	...
	RTE_LOG(INFO, USER1, "Drain tx queue period: %d us\n",
		burst_tx_drain_us);
