...
def plot_bw():
    ...
    for idx, csv in enumerate(csv_names):
        ...
        bw_arr = (
            np.genfromtxt(csv_path, delimiter=",", usecols=list(range(0, 2))) / 1000.0
        )
        bw_avg = bw_arr[:, 1]
...
def plot_adv_comp_per_packet_delay(payload_size=1400):
    ...
    func_map = {
        "nc_enc_sliding_window_25_binary8": [0, "NC Sliding Window", "++++", "-", "o"],
        ...
        for func, para in func_map.items():
            ...
            for i in range(1, N + 1):
                if payload_size != 1400:
                    csv_files.append(
                        "{}_per_packet_delay_{}_{}.csv".format(i, func, payload_size)
                    )
                else:
                    csv_files.append("{}_per_packet_delay_{}.csv".format(i, func))
            delay_map = dict()
            delay_map_val = list()
            for csv in csv_files:
                ...
                delay_map_val.append((np.average(delay_arr), calc_std_err(delay_arr)))
