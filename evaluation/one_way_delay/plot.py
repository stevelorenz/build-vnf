...
def save_fig(fig, path, fmt=FIG_FMT):
    """Save fig to path"""
    fig.savefig(
        path + ".%s" % fmt, pad_iches=0, bbox_inches="tight", dpi=400, format=fmt
    )
...
def plot_ipd():
    ...
    labels = ("Kernel IP Forwarding", "DPDK Forwarding", "DPDK Appending Timestamps")
