 ...
 *
 * About : Network Coding Encoder for UDP packets
 *
 * MARK  : - Packets are all received and sent in Layer 2 with MAC addresses
 *		   - Current uses only one core for all operations
 *
 * Dep   : - DPDK
 *		   - kodo-c
 *
 * TODO  : - Add multi-process support to utilize the multi-core feature of DPDK
 *				Plan: core0: rx, core1: coding, core2: tx
 */
...
/* Encoding packets enabled by default */
 ...
 * Can be used as the generation size for coding
 * */
...
/* KODO encoder */
static kodoc_coder_t encoder;
/* Buffer for to be coded packets */
static struct rte_mbuf encode_buffer[GENERATION_SIZE];
...
/* Encode UDP payload with nckernel */
...
l2fwd_main_loop(void)
{
	...
	/* Packets that are passed to the encoder for encoding */
	struct rte_mbuf *pkts_encode[GENERATION_SIZE];
	struct rte_mbuf *m;
	int sent;
	unsigned lcore_id;
	uint64_t prev_tsc, diff_tsc, cur_tsc;
	unsigned i, j, k, portid, nb_rx;
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			/* TODO(zuo): This part SHOULD be accelerated with packet filter feature of DPDK
			 * Filer out non-UDP packets in the pkts_burst buffer, put UDP
			 * packet index into the encode buffer
			 * */
			k = 0;
			if (likely(nb_rx > 0)) {
				for (j = 0; j < nb_rx; ++j) {
					m = pkts_burst[j];
					if ( unlikely(!(is_ipv4_pkt(m)) || !(is_udp_dgram(m))) ) {

						RTE_LOG(DEBUG, USER1, "Frame idx:%d is not a UDP datagram.\n", j);
						continue;
					}
					else{
			...
			/*RTE_LOG(DEBUG, USER1, "Frames proc, TSC: %ld, time:%f ms\n", proc_tsc, proc_time);*/
			/*}*/
...
#define CMD_LINE_OPT_ENCODING "encoding"
#define CMD_LINE_OPT_NO_ENCODING "no-encoding"
...
static const struct option lgopts[] = {
	...
	{ CMD_LINE_OPT_ENCODING, no_argument, &encoding, 1},
	{ CMD_LINE_OPT_NO_ENCODING, no_argument, &encoding, 0},
	{NULL, 0, 0, 0}
};
...
main(int argc, char **argv)
{
	...
	RTE_LOG(INFO, USER1, "Encoding: %s\n", encoding? "enabled" : "disabled");

	if (encoding) {
		/* Initialise KODO encoder TODO: parse args in CLI */
		uint32_t max_symbols = GENERATION_SIZE;
		uint32_t max_symbol_size = MAX_UDP_DATA_LEN;


		/* Select the codec we wish to use */
		int32_t codec = kodoc_full_vector;

		/* Select the finite field to use. */
		/* Some common choices are: kodoc_binary, kodoc_binary4, kodoc_binary8 */
		int32_t finite_field = kodoc_binary;

		/* Build the encoder factory */
