...
function help() {
    echo "\$run_app.sh proc_opt max_pkt_burst drain_tx_us add_args"
    ...
    echo "-add_args: Additional arguments"
    echo "  --(no-)single-port: En(dis)able forwarding with single port"
}
