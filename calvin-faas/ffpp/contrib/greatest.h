	...
	do {                                                                   \
		...
		if (!greatest_do_assert_equal_t(EXP, GOT, type_info, UDATA)) { \
			if (type_info == NULL || type_info->equal == NULL) {   \
				GREATEST_FAILm(                                \
					"type_info->equal callback missing!"); \
			} else {                                               \
				GREATEST_FAILm(MSG);                           \
			}                                                      \
		}                                                              \
 ...
 * test shuffling state, if applicable, and then call setup hooks. */        \
	...
	{                                                                                \
		...
		if (match &&                                                             \
		    (!GREATEST_FIRST_FAIL() || g->suite.failed == 0)) {                  \
	...
	{                                                                                \
		...
		return GREATEST_FPRINTF(GREATEST_STDOUT, "%s",                           \
					(const char *)t);                                \
	}                                                                                \
                                                                                         \
