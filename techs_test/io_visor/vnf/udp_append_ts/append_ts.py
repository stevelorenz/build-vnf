...
def main():
    ...
    recv_sock = socket.fromfd(
        recv_sock_fd, socket.AF_PACKET, socket.SOCK_RAW, socket.htons(3)
    )
    ...
    try:
        while True:
            ...
            # IP total length
            old_ip_tlen = struct.unpack(">H", eth_frame[hd_offset + 2 : hd_offset + 4])[
                0
            ]
            ...
            # UDP payload length
            old_udp_pl_len = (
                struct.unpack(">H", eth_frame[hd_offset + 4 : hd_offset + 6])[0]
                - UDP_HDL
            )

            # Append recv time stamp at end
            eth_frame[
                udp_pl_offset + old_udp_pl_len : udp_pl_offset + old_udp_pl_len + ts_len
            ] = recv_time_b

            # Set UDP and IP total length with new payload
            new_udp_tlen = struct.pack(">H", (old_udp_pl_len + UDP_HDL + ts_len))
            ...
            pseudo_udp_header = struct.pack(
                "!BBH", 0, socket.IPPROTO_UDP, old_udp_pl_len + ts_len + UDP_HDL
            )
            ...
            udp_cksum = calc_udp_cksum(
                pseudo_udp_header
                + udp_header
                + eth_frame[udp_pl_offset : udp_pl_offset + old_udp_pl_len + ts_len]
            )
            eth_frame[hd_offset + 6 : hd_offset + 8] = struct.pack(">H", udp_cksum)

            # Move back to IP header
            hd_offset -= ihl
            new_ip_tlen = struct.pack(">H", (old_ip_tlen + ts_len))
            ...
            new_iph_cksum = calc_ih_cksum(eth_frame[hd_offset : hd_offset + ihl])
            eth_frame[hd_offset + 10 : hd_offset + 12] = struct.pack(
                "<H", new_iph_cksum
            )
            frame_len += ts_len
