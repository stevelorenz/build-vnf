...
class Base(object):
    def __init__(self, name):
        self._name = name
        self._send_thd = threading.Thread(
            daemon=True, target=self._send_pack, name=self._name + "_send_thd"
        )
        self._recv_thd = threading.Thread(
            daemon=True, target=self._recv_pack, name=self._name + "_recv_thd"
        )
        self._send_sock = socket.socket(family=socket.AF_INET, type=socket.SOCK_DGRAM)
        self._recv_sock = socket.socket(family=socket.AF_INET, type=socket.SOCK_DGRAM)
    ...
    def run(self):
        ...
        try:
            ...
            logger.info("[%s] KeyboardInterrupt detected, run cleanups.", self._name)
...
class UDPClient(Base):
    ...
    def __init__(self, send_addr, recv_ip, pack_nb, ipd, pld_size, b_addr, push=False):
    ...
    def _send_pack(self):
        ...
        logger.info(
            "[Client] Start sending UDP packets to %s:%s"
        ...
        while send_idx < self._pack_nb:
            send_data[0:16] = struct.pack("!qQ", send_idx, int(time.time() * 1e6))
            st_send_ts = time.time()
            self._send_sock.sendto(
                send_data[0 : self._pld_size], (self._send_ip, self._send_port)
            )
            send_dur = time.time() - st_send_ts
            logger.debug(
                "[Client] send index:%d, send duration: %f" % (send_idx, send_dur)
            )
        ...
        if self._push:
            logger.info("[Client] Send additional %d pushing packets.", PUSH_PACK_NUM)
            send_idx = 0
            while send_idx < PUSH_PACK_NUM:
                send_data[0:16] = struct.pack("!qQ", -1, int(time.time() * 1e6))
                self._send_sock.sendto(
                    send_data[0 : self._pld_size], (self._send_ip, self._send_port)
                )
    ...
    def _recv_pack(self):
        ...
        logger.info(
            "[Client] Start receiving UDP packets from %s:%s"
        ...
        logger.info("[Client] Receive all bounced packets, start calculating RTTs.")
    ...
    def _calc_rtt(self):
        ...
        with open(CSV_FILE_PATH, "a+") as csv_file:
            csv_file.write(",".join(map(str, self._rtt_result)))
            csv_file.write("\n")
    ...
    def _recv_pack(self):
        ...
        while True:
            ...
            if self.buffer.full():
                ...
                pcap_file_name = "queue_overflow_{}.pcap".format(int(time.time()))
            ...
            if not self.buffer.empty():
                ...
                if send_idx < 0:
                    ...
                    logger.info(
                        ...
                        "[Server] Delay %f seconds before bouncing the first packet",
                        self._send_delay,
                    )
                ...
                self._send_sock.sendto(data_arr[:], (addr[0], (self._recv_port + 1)))
                logger.debug(
                    "[Server] Bounce index: %d, queue time: %dus",
                    bounce_idx,
                    queue_time,
                )
                bounce_idx += 1
...
if __name__ == "__main__":
    ...
    group.add_argument("-s", metavar="ADDRESS", type=str, help="Run as UDP server.")
    parser.add_argument(
        "--srv_queue_len",
        type=int,
        default=100,
...
chain. (Bug of neutron sfc extension)""",
    ...
    parser.add_argument(
        ...
        metavar="IP",
        type=str,
        help="IP used by client to receive packets." + "Listen port = send port + 1",
    )

    parser.add_argument("-n", type=int, default=3, help="Number of to be sent packets.")
    parser.add_argument(
        "--ipd", type=float, default=1.0, help="Inter-packet delay in seconds."
    )
    parser.add_argument(
        "--payload_size", type=int, default=256, help="UDP Payload size in bytes."
    )
    parser.add_argument(
        "--csv_path", type=str, default="./udp_rtt.csv", help="Path to store CSV files."
    )
    parser.add_argument(
        "--bind",
        metavar="ADDRESS",
    ...
    parser.add_argument(
        "--log", type=str, default="INFO", help="Logging level, e.g. INFO, DEBUG"
    )
    args = parser.parse_args()

    logging.basicConfig(
        level=args.log, handlers=[logging.StreamHandler()], format=fmt_str
    )
    ...
    if args.c:
        ...
        port = int(port)
        udp_clt = UDPClient(
            (ip, port), args.l, args.n, args.ipd, args.payload_size, b_addr, args.push
        )
        logger.info(
            "IPD: %f, Payload size: %d, Push: %s",
            args.ipd,
            args.payload_size,
            args.push,
        )
    ...
    if args.s:
        ...
        port = int(port)
        logger.info(
            "Server queue length: {}, send delay: {}s".format(
                args.srv_queue_len, args.delay
            )
        )
        udp_srv = UDPServer(
            args.srv_queue_len, (ip, port), args.delay, args.payload_size, b_addr
        )
        udp_srv.run()
