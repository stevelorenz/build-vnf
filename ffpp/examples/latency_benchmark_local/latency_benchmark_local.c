...
static int proc_loop_slave(__attribute__((unused)) void *dummy)
{
	...
	for (r = 0; r < TEST_ROUND; r++) {
		...
		for (img_idx = 0; img_idx < IMAGE_FRAME_NUM; img_idx++) {
			...
			rte_ring_dequeue_bulk(l2v_que, (void **)rt_buf, nb_mbuf,
					      NULL);
       ...
       */
			...
			*(uint16_t *)(meta_data) = nb_mbuf;
			...
			rte_ring_enqueue_bulk(v2l_que, (void **)rt_buf, nb_mbuf,
					      NULL);
		}
	}
	close(socket);
	...
	for (r = 0; r < TEST_ROUND; r++) {
		...
		for (img_idx = 0; img_idx < IMAGE_FRAME_NUM; img_idx++) {
			...
			nb_mbuf = gen_rx_buf_from_file("./pikachu.jpg", rt_buf,
						       RT_BUF_SIZE, mbuf_pool,
						       1500, &tail_size);
			RTE_LOG(INFO, TEST,
				"Delay for generate mbufs from file:%.8f ms\n",
				get_delay_tsc_ms(rte_get_tsc_cycles() -
						 last_tsc));
			RTE_LOG(INFO, TEST,
			...
			*(uint16_t *)(meta_data) = nb_mbuf;
			...
			rte_ring_enqueue_bulk(l2v_que, (void **)(tx_vec->head),
					      nb_mbuf, NULL);
       ...
       * ring */
			...
			RTE_LOG(INFO, FPP,
				...
				nb_mbuf);
			rte_mempool_put(mbuf_pool, meta_data);
			rte_ring_dequeue_bulk(v2l_que, (void **)rt_buf, nb_mbuf,
					      NULL);
		...
		save_double_list_csv("./proc_delay_ms.csv", proc_delay_ms_list,
				     IMAGE_FRAME_NUM);
		save_double_list_csv("./total_delay_ms.csv", proc_delay_ms_list,
				     IMAGE_FRAME_NUM);
		save_u64_list_csv("./proc_tsc.csv", proc_tsc_list,
				  IMAGE_FRAME_NUM);
		save_u64_list_csv("./total_tsc.csv", total_tsc_list,
				  IMAGE_FRAME_NUM);
	}
