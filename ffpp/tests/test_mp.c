...
static int proc_loop_prim(__attribute__((unused)) void *dummy)
{
	...
	nb_mbuf = gen_rx_buf_from_file("./pikachu.jpg", rx_buf, RX_BUF_SIZE,
...
int main(int argc, char *argv[])
{
	...
	if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
		...
		send_ring =
			rte_ring_create("PRI2SEC", ring_size, rte_socket_id(),
					RING_F_SP_ENQ | RING_F_SC_DEQ);
		recv_ring =
			rte_ring_create("SEC2PRI", ring_size, rte_socket_id(),
					RING_F_SP_ENQ | RING_F_SC_DEQ);
		mbuf_pool =
		...
		struct dpdk_device_config cfg = { 0,   &mbuf_pool, 1, 1,
						  256, 256,	1, 1 };
